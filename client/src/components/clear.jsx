...
export const ClearButton = () => {
    const initialDocumentState = [
        { id: 1, Project: "", DocumentNo: "", DocumentDescription: "", Rev: "", Prepared: "", Checked: "", Approved: "", Remarks: "", editable: true, selected: false },
    ];
    ...
    const handleFileUpload = (event) => {
        ...
        reader.onload = (e) => {
            ...
            if (data.length > 1) { // Check if there are data rows (excluding header)
                // Filter out empty rows and header row
                const filteredData = data.slice(1).filter(row => row.some(cell => cell !== ''));
    ...
    const handleDone = () => {
        ...
        for (const doc of documents) {
            if (!doc.Project || !doc.DocumentNo || !doc.DocumentDescription || !doc.Rev || !doc.Prepared || !doc.Checked || !doc.Approved || !doc.Remarks) {
        ...
        const updatedDocuments = documents.map(doc => ({
            ...
            Project: typeof doc.Project === 'object' ? <span>{doc.Project.props.children}</span> : doc.Project,
        }));
    ...
    return (
        ...
        <div className="container2">
            ...
            <button className={showDeleteButton ? 'DeleteButton' : 'ClearButton'} onClick={handleClearOrDelete}>
